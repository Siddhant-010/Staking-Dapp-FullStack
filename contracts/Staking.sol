...
contract Staking{
    ...
    function calculateInterest(uint basispoints,uint numDays, uint weiAmount) private pure returns(uint){
        return basispoints*weiAmount/10000;  // 700/10000= 0.07=>7%
    }
    ...
    function changeUnlockDate(uint positionId,uint newUnlockDate) external{
        require(owner==msg.sender,"Only owner may modify unlock periods");
        positions[positionId].unlockDate=newUnlockDate;
    }
    function closePositions(uint positionId) external {
        require(positions[positionId].walletAddress==msg.sender,"Only position creater may modify positions");
