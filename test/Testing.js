...
describe("Staking", function () {
  ...
  describe("stakeEther", function () {
    it("transfers ether", async function () {
      ...
      // test the change in signer1's eteher balance
    ...
    it("adds a position to positions", async function () {
      ...
      expect(await staking.currentPositionId()).to.equal(false);
    ...
    it("adds address and positionId to positionIdsByAddress", async function () {
      ...
      expect(await staking.positionIdsByAddress(signer1.address, 1)).to.equal(
        1
      );
  ...
  describe("getPositionIdsForAddress", function () {
    it("returns a list of positionIds created by a specific address", async () => {
      ...
      const positionIds = await staking.getPositionIdsForAddress(
        signer1.address
      );
  ...
  describe("changeUnlockDate", function () {
    ...
    describe("non-owner", function () {
      it("reverts", async function () {
        ...
        ).to.be.revertedWith("Only owner may modify unlock periods");
      });
    });
  });
  describe("closePositions", function () {
    describe("after unlock date", function () {
      it("transfers principal and interst", async () => {
        let transaction;
        let receipt;
        let block;
        const provider = waffle.provider;
        ...
        transaction = await staking.connect(signer2).closePositions(0);
        receipt = await transaction.wait();
